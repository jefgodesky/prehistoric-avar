...
describe('Markable', () => {
  ...
  describe('constructor', () => {
    ...
    it('can take an ID', () => {
      const mark = new Markable(sim, { id })
      expect(mark.id).toBe(id)
    })

    it('can take markers', () => {
      const mark = new Markable(sim, { markers })
  ...
  describe('Member methods', () => {
    describe('addMarker', () => {
      ...
      it('emits an event', async () => {
        ...
        sim.emitter.on(MARKABLE_EVENTS.MARKED, ({ id, marker }: { id: string, marker: string }) => {
          emitted = `${id}: ${marker}`
        })
    ...
    describe('removeMarker', () => {
      ...
      it('emits an event', async () => {
        let emitted: boolean | string = false
        sim.emitter.on(MARKABLE_EVENTS.UNMARKED, ({ id, marker }: { id: string, marker: string }) => {
          emitted = `${id}: ${marker}`
        })
  ...
  describe('Event handling', () => {
    describe(MARKABLE_EVENTS.ADD, () => {
      ...
      it('adds a marker', async () => {
        const mark = new Markable(sim, { id, markers: [] })
        await sim.emitter.emit(event, { id, marker })
        expect(mark.markers).toHaveLength(1)
      })
    })

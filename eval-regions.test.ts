...
Hot deserts:
  ...
  it('calculates area for each region', () => {
    const result = evalRegions(input, data)
    expect(result.F01.area).toBe(526283)
    expect(result.D01.area).toBe(2321682)
  })

  it('calculates carrying capacity correctly', () => {
    const result = evalRegions(input, data)
    expect(result.F01.carryingCapacity).toBe(63153)
    expect(result.D01.carryingCapacity).toBe(92867)
  })
